...
| D1 | P0 | Produce and validate sprite frames | Grace (image generation) + tools | None | Yes | `DONE` | 2026-09-21 | 25 frames + retimed manifest in `res/sprites/baiyuan/` @ `eba772d` · `python3 tools/sprite_check.py res/sprites/baiyuan` → 0 errors, 25 warnings (all bust-shot bottom edges, expected) · head width 171–186px across all frames, redrawn 8 at 177–181px · visual review passed for all 26 source frames (MissionCrew docs `reviews/d1-sprite-visual-audit.md`, `reviews/d1-redraw-comparison.png`) · `npm test` 112/112 · 2026-09-21. In-app playback is verified by exit criterion 2 and C2, not here |
