...
| G1 | P0 | Replace the hard onboarding gate with a "configure later" path | architect, electron-dev, frontend-dev | None | No | `REVIEW` | 2026-09-17 | `23a56d2` (Agent + main) + `adbb49a` (onboarding + renderer), spec `v0.2.0/SPEC-G1-SOFT-GATE.md` @ `b9c0f7d` · lead re-ran on the working tree before commit: `npx tsc` ×2 exit 0, `npm test` 11 suites / 106 tests, `npm run build:renderer` ✓ 116 modules. Independent review found no P0 and one P1 (a `cpa` field missing from `settings.json` threw inside `launchMainApp()`, leaving the main UI half-started); fixed with the two P2s in `9177009` + `41a0776`, docs backfilled in `fb3b7c1`. Lead re-ran: `npx tsc` ×2 exit 0, `npm test` 11 suites / 112 tests, `build:main` + `build:renderer` exit 0. Open: re-review of the fix; GUI acceptance M1–M11 |
