...
| G1 | P0 | Replace the hard onboarding gate with a "configure later" path | architect, electron-dev, frontend-dev | None | No | `REVIEW` | 2026-09-17 | `23a56d2` (Agent + main) + `adbb49a` (onboarding + renderer), spec `v0.2.0/SPEC-G1-SOFT-GATE.md` @ `b9c0f7d` · lead re-ran on the working tree before commit: `npx tsc` ×2 exit 0, `npm test` 11 suites / 106 tests, `npm run build:renderer` ✓ 116 modules. Open: independent review; two known P2s (skip-hint style; `isLlmConfigured()` throws if a `cpa` field is missing from a hand-edited `settings.json`); GUI acceptance M1–M8 |
