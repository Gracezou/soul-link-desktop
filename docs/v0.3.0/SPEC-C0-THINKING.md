...
- **未闭合的情况**（2026-09-16 收紧）：出现未闭合的思维标签时，**从该标签一直删到文本结尾**，
  不要求它位于开头。原文写的是「以 `<think>` 开头才处理」，实现（`2eb304c`）比这激进，
  审查确认这个偏离是对的：`正文<think>半截` 这种形态下，按原写法标签会直接入库，
  正是本任务要堵的洞。代价是正文里合法出现未闭合标签时后半段会被吃掉
  （`"你可以写 <think> 标签"` → `"你可以写 "`）；权衡的方向是**宁可丢内容，不可产生污染**。
  文本整段都是未闭合思维链时，结果自然是空串。
  必须有一条 `'before<think>unfinished'` 用例锁住这个行为——现有用例全是「整段都是思维链」，
  恰好绕开了这条分歧
- 不含标签时**原样返回**（不要 trim 掉正常内容的首尾结构，`*动作*` 依赖前后空白）
- **孤立闭合标签**（2026-09-16 新增）：只有 `</think>` 没有开标签时（部分网关会吞掉开标签），
  删除到最后一个闭合标签为止。与未闭合规则同向：宁可丢内容，不可产生污染
- 纯函数，不依赖 Electron

**已知且有意接受的副作用**（实测于 `a206559`，不要当 bug 修）：正文里合法提到思维标签时会被吃掉。

| 输入 | 输出 |
|---|---|
| `正文前<think>未闭合` | `正文前` |
| `推理内容</think>正文` | `正文` |
| `回答：写 </think> 可以结束思考块` | ` 可以结束思考块` |

最后一行是代价所在——桌宠聊到标签本身时会丢掉前半句。权衡的依据是：污染不可逆（进库、每轮回灌），
丢内容可见且只影响当次回复。
...
4. ~~同等对话轮次下 `context.tokenEstimate` 显著下降（对照基线：10 条消息 4445）~~
   **这个验收项本身写错了，2026-09-16 作废并替换。** 实测发现 `historyLength=10` 不构成同等上下文：
   旧基线那 10 条是 8 user + 2 assistant，修复后实测是 5 + 5，assistant 内容更多，
   于是 `tokenEstimate` 反而升到 4671。这不是回归。
   **正确的隔离方法是反事实计算**：对同一批旧数据做剥离前后对照——
   旧库 assistant 正文里思维链占 **46.3%** 的字符，整体上下文 `4445 → 3916`（-11.9%）。
   今后要做 token 对照，必须固定消息条数与角色配比，或者直接用反事实算法
